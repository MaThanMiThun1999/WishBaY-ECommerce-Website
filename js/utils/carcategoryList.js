...
let categoryCar = document.querySelector(".categorycarProduct");
...
function carloadCategory() {
  let car = products.filter((product) => {
    return product.category.some((el) => el.name === "Car Electronics & GPS" || el.name === "Car Installation Parts & Accessories" || el.name === "More Car Accessories");
  });

  let carCategoryHtml = "";
  car.forEach((category) => {
    carCategoryHtml += `
                ...
                <div class="rounded-xl border-[1px] border-[rgb(220, 229, 233)] hover:scale-105 duration-200">
                  ...
                  <div class="px-2">
                    <h1 class="font-bold md:text-base text-xs truncate mt-3 md:mt-5 mb-1">${category.name}</h1>
                    <div class="flex items-center space-x-1 pb-1">
                    <img class="md:w-20 w-14" src="./assets/ratingImages/rating-${category.rating * 10}.png" alt="" srcset="">
                      <span class="text-gray-700 mt-[2px] text-sm font-medium">
                        ${category.rating}
                      </span>
                    </div>
                    <h5 class="pb-2 font-bold">$ ${centsToDollars(dollarsToCents(category.price)).toFixed(2)}</h5>
                    <div class="pb-2">
                    <button
  ...
  categoryCar.innerHTML = carCategoryHtml;
}
